...
static int ps8xxx_tcpc_init(const struct device *dev)
{
	...
	if (!data->initialized) {
		LOG_ERR("TCPC is not initialized correctly");
		return -EIO;
	}

	LOG_INF("TCPC initialized");
	return 0;
}

int tcpci_tcpm_get_cc(const struct device *dev, enum tc_cc_voltage_state *cc1,
		      enum tc_cc_voltage_state *cc2)
{
	const struct ps8xxx_cfg *cfg = dev->config;
	struct ps8xxx_data *data = dev->data;
	...
	*cc2 = TC_CC_VOLT_OPEN;
	...
	rv = tcpci_read_reg8(&cfg->bus, TCPC_REG_ROLE_CTRL, &role);
	if (rv) {
		return rv;
	}

	rv = tcpci_read_reg8(&cfg->bus, TCPC_REG_CC_STATUS, &status);
	if (rv) {
		return rv;
	}
		       ...
		       enum tc_cc_voltage_state *cc2)
{
	...
	return tcpci_tcpm_get_cc(dev, cc1, cc2);
}
...
int ps8xxx_tcpc_set_vconn(const struct device *dev, bool enable)
{
	...
	ret = tcpci_update_reg8(&cfg->bus, TCPC_REG_POWER_CTRL, TCPC_REG_POWER_CTRL_VCONN(1),
				TCPC_REG_POWER_CTRL_VCONN(enable));
...
int ps8xxx_tcpc_receive_data(const struct device *dev, struct pd_msg *msg)
{
	const struct ps8xxx_cfg *cfg = dev->config;
	struct ps8xxx_data *data = dev->data;
...
int ps8xxx_tcpc_is_rx_pending_msg(const struct device *dev, enum pd_packet_type *type)
{
	struct ps8xxx_data *data = dev->data;

	return data->msg_pending;
}
...
int ps8xxx_tcpc_set_cc_polarity(const struct device *dev, enum tc_cc_polarity polarity)
{
	...
	ret = tcpci_update_reg8(&cfg->bus, TCPC_REG_TCPC_CTRL, TCPC_REG_TCPC_CTRL_SET(1),
...
int ps8xxx_tcpc_transmit_data(const struct device *dev, struct pd_msg *msg)
{
	...
	rv = tcpci_write_reg8(&cfg->bus, TCPC_REG_TRANSMIT,
			      TCPC_REG_TRANSMIT_SET_WITH_RETRY(
				      CONFIG_USBC_TCPC_TCPCI_TRANSMIT_RETRIES, msg->type));
...
int ps8xxx_tcpc_get_chip_info(const struct device *dev, struct tcpc_chip_info *chip_info)
{
	...
	/* TODO: ps8815 only */
	ret |= tcpci_read_reg16(&cfg->bus, 0x82, &fw_ver);
	chip_info->fw_verion_number = fw_ver;
...
static const struct tcpc_driver_api ps8xxx_driver_api = {
	...
	.receive_data = ps8xxx_tcpc_receive_data,
	.is_rx_pending_msg = ps8xxx_tcpc_is_rx_pending_msg,
...
static int ps8xxx_dev_init(const struct device *dev)
{
	...
	ready = device_is_ready(cfg->bus.bus);
	if (!ready) {
		return -EIO;
	}
...
#define PS8XXX_DRIVER_CFG_INIT(node)                                                               \
	{                                                                                          \
		.bus = I2C_DT_SPEC_GET(node), .alert_gpio = GPIO_DT_SPEC_GET(node, irq_gpios),     \
	}
...
#define PS8XXX_DRIVER_INIT(inst)                                                                   \
	...
	DEVICE_DT_INST_DEFINE(inst, &ps8xxx_dev_init, NULL, &drv_data_ps8xxx##inst,                \
			      &drv_cfg_ps8xxx##inst, POST_KERNEL, CONFIG_USBC_INIT_PRIORITY,       \
			      &ps8xxx_driver_api);
